...
data Trace l t = Trace [l] (Maybe t)

instance Monad (Trace l) where
    return x = Trace [] (Just x)
    -- :: Trace l t -> (t -> Trace l u) -> Trace l u
    Trace msgs (Just v) >>= f = let Trace msgs' w = f v in
                                Trace (msgs ++ msgs') w
    Trace msgs Nothing >>= _ = Trace msgs Nothing

trace :: l -> Trace l ()
trace msg = Trace [msg] (Just ())

censor :: (l -> l) -> Trace l t -> Trace l t
censor f (Trace msgs v) = Trace (map f msgs) v
    ...
    where eval' _ (EInt x) =
          ...
          eval' h (EAdd e1 e2) =
              do VInt x1 <- recursiveEval h e1
                 ...
                 return (VInt (x1 + x2))
{-
              recursiveEval h e1 >>= \ (VInt x1) ->
              recursiveEval h e1 >>= \ (VInt x2) ->
                 return (VInt (x1 + x2))
-}
          eval' h (ESubt e1 e2) =
              do VInt x1 <- recursiveEval h e1
...
eval :: VEnv -> Expr -> Trace String Value
eval h (EThrow t) = Trace [show (EThrow t)] Nothing
eval h (ECatch e1 e2) =
    do v <- censor (\s -> "  " ++ s)
            (case eval h e1 of
               Trace msgs Nothing ->
                   let Trace msgs' v = eval h e2 in
                   Trace (msgs ++ msgs') v
               Trace msgs (Just v) -> Trace msgs (Just v))
       trace (show (ECatch e1 e2) ++ "    ===>    " ++ show v)
       return v
eval h e = do v <- censor (\s -> "  " ++ s) (geval eval h e)
              trace (show e ++ "    ===>    " ++ show v)
              return v
...
parse = runParser (terminal exprp) () ""
    where l = makeTokenParser $
              haskellDef { reservedNames = ["True", "False", "if", "then", "else",
                                            "let", "in", "Int", "Bool", "try", "catch", "throw"]
                         , reservedOpNames = ["+", "*", "\\", "->", ":", ","] }
...
go s = let result = do e <- parse' s
                       _ <- check' e
                       let Trace msgs v = eval [] e
                       return (unlines (msgs ++ [show v]))
       in case result of
...
instance Functor (Trace l) where
    fmap f = (>>= return . f)

instance Applicative (Trace l) where
    pure = return
    (<*>) = liftM2 ($)
