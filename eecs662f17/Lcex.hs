...
data Type = TInt | TBool | TFun Type Type | TPair Type Type
  deriving (Eq, Show)
...
data Value = VInt Integer | VBool Bool | VFun VEnv Ident Expr | VPair Value Value
  deriving (Eq, Show)
data Result = Ok Value | Exception
  deriving (Eq, Show)

type VEnv = [(Ident, Value)]

andThen :: Result -> (Value -> Result) -> Result
Exception `andThen` _ = Exception
Ok v `andThen` f      = f v
...
eval _ (EInt i) =
    Ok (VInt i)
eval h (EAdd e1 e2) =
    eval h e1 `andThen` \ (VInt x1) ->
    eval h e2 `andThen` \ (VInt x2) ->
        Ok (VInt (x1 + x2))
eval h (ESubt e1 e2) =
    eval h e1 `andThen` \ (VInt x1) ->
    eval h e2 `andThen` \ (VInt x2) ->
...
eval _ (EBool b) =
    Ok (VBool b)
eval h (EIs0 e) =
    eval h e `andThen` \ (VInt x) ->
        Ok (VBool (x == 0))
eval h (EIf e e1 e2) =
    eval h e `andThen` \ (VBool b) ->
...
eval h (ELet x e1 e2) =
    eval h e1 `andThen` \ v ->
        eval ((x, v) : h) e2
eval h (EPair e1 e2) =
    eval h e1 `andThen` \ v1 ->
    eval h e2 `andThen` \ v2 ->
        Ok (VPair v1 v2)
eval h (ELetPair x1 x2 e1 e2) =
    eval h e1 `andThen` \ (VPair v1 v2) ->
