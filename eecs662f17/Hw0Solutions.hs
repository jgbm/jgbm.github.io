...
We will represent points as pairs of double-precision floating point numbers,
and circles as a pair of the center (a point) and the radius (a Double).
  ...
  b) The function cirlceIntersectsCircle, given two circles, should return True
...
pointInCircle ((x0, y0), r) (x, y) = sqrt (sqr (x - x0) + sqr (y - y0)) <= r
...
circleIntersectsCircle ((x0, y0), r0) ((x1, y1), r1) =
    sqrt (sqr (x1 - x0) + sqr (y1 - y0)) <= r0 + r1
...
sumSquares [] = 0
sumSquares (x:xs) = sqr x + sumSquares xs
...
sumSquaredEvens (x:xs)
    | even x = sqr x + sumSquaredEvens xs
    | otherwise = sumSquaredEvens xs
...
secondLargest (x : y : zs) = go x' y' zs
    where x' = max x y
          y' = min x y
          go x y [] = y
          go x y (z : zs)
...
sumSquaredEvens' = foldl (+) 0 . map sqr . filter even
...
isSorted' :: [Int] -> Bool
isSorted' [] = True
isSorted' (x : xs) = snd (foldl f (x, True) xs)
    where f (y, b) z = (z, y <= z && b)
...
secondLargest' (x : y : zs) = snd (foldl f (x', y') zs)
    where x' = max x y
          y' = min x y
          f (x, y) z
              | z > x          = (z, x)
...
We have provided a function readHand, which transforms strings of characters
into lists of cards.  Aces through Tens are represented by the first letter of
their names, while other cards by their numeric value.  For example, readHand
"at4" will return [Ace, Rank 10, Rank 4].
...
blackjackTests =
    TestList [ "dealer bust" ~: readHand "aj9" `beats` readHand "kj9" ~?= True
             , "natural" ~: readHand "aj" `beats` readHand "ajk" ~?= True
             -- Your tests go here
             ]
...
player `beats` dealer
    | isNatural dealer || null playerScores = False
    | isNatural player || null dealerScores = True
    | otherwise = maximum playerScores > maximum dealerScores
    where isNatural hand = length hand == 2 && maximum (scores hand) == 21

          scoreCard Ace = [1,11]
          scoreCard (Rank n) = [n]
          scoreCard _ = [10]

          scores = filter (<= 21) . map sum . mapM scoreCard
          playerScores = scores player
          dealerScores = scores dealer
...
For testing, we have provided a function isDnf, which returns True if its input
...
dnf :: Formula -> Formula
-- Atoms and negated atoms are already DNF
dnf (Atom x) = Atom x
dnf (Not (Atom x)) = Not (Atom x)
-- Double negation
dnf (Not (Not phi)) = dnf phi
-- De Morgan
dnf (Not (phi :/\: psi)) = dnf (Not phi :\/: Not psi)
dnf (Not (phi :\/: psi)) = dnf (Not phi :/\: Not psi)
-- Conjunctions: conjuncts of conjunts are okay; conjuncts of disjuncts need to
-- be distributed.
dnf (phi :/\: psi) =
    case (phi', psi') of
      (phi1 :\/: phi2, _) ->
          dnf ((phi1 :/\: psi') :\/: (phi2 :/\: psi'))
      (_, psi1 :\/: psi2) ->
          dnf ((phi' :/\: psi1) :\/: (phi' :/\: psi2))
      _ -> phi' :/\: psi'
    where phi' = dnf phi
          psi' = dnf psi
-- Disjunctions: are cool, but need to have their arguments dnf'd as well
dnf (phi :\/: psi) = dnf phi :\/: dnf psi
