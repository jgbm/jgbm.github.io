...
check _ (EInt _) =
    return TInt
check g (EAdd e1 e2) =
    check g e1 `andThen` \ TInt ->
    check g e2 `andThen` \ TInt ->
        ok TInt
{-
    do TInt <- check g e1
       TInt <- check g e2
...
-- data ExceptionResult t = Ok t | Exception
data ExceptionResult t = Ok t | Exception
  deriving (Eq, Show)
...
class Result r where
    ok :: t -> r t
    andThen :: r t -> (t -> r u) -> r u

instance Result ExceptionResult where
    ok v = Ok v
...
instance Result Maybe where
    ...
    Just v `andThen` f  = f v

geval :: (Result r, Monad r) =>
         (VEnv -> Expr -> r Value) ->
         VEnv -> Expr -> r Value
